...
// Version 3: Tiled [i,j,k]
void matmul_tiled_ijk(double **A, double **B, double **C, int n, int tile) {
...
int main() {
    ...
    // Tiled [i,j,k]
    printf("Running matmul_tiled_ijk ...\n");
    start = get_time();
    matmul_tiled_ijk(A, B, C, N, TILE_SIZE);
    end = get_time();
    printf("Time (tiled [i,j,k]): %f seconds\n", end - start);
